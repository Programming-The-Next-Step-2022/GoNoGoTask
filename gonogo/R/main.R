...
play_gonogo <- function(id, n_trial = 40, n_block = 4, stimuli = c("A", "X"), 
  ...
  if (class(n_trial) != "numeric" || length(n_trial) != 1 || 
    ...
    stop("n_trial must be a numeric (integer and positive) vector of length 1")
  }
  if (class(n_block) != "numeric" || length(n_block) != 1 ||
      n_block %% 1 != 0 || n_block < 0) {
    stop("n_block must be a numeric (integer and positive) vector of length 1")
  }
  ...
  for (p in 1:10) {
    ...
    rrt(choice_keys = c(" "), interval = inter)
  }
  ...
  for (j in 1:n_block) {
    ...
    for (i in 1:n_trial) {
      ...
      all_data[[j]][i, c("response", "rt")] <- rrt(choice_keys = c(" "), 
                                                  interval = inter)
    }
    draw_screen(paste0("End of Block ", j, "!\n Press Enter to continue."),
               cex = 1)
    readline()
  }
  
  # End screen
  draw_screen(paste0("You finished the task. Thank you for participating, ",id,"!"), cex = 1)
